...
export function responseInterceptor(): MiddlewareHandler<Env> {
  return async (c: Context<Env>, next) => {
    ...
    const skipPaths = ['/docs', '/openapi.json', 'jpg', 'jpeg', 'png', 'mp4', 'mov', 'csv']
    ...
    if (contentType && (contentType.startsWith('video/') || contentType.startsWith('image/') || contentType === 'text/csv')) {
